...
import {
  guessMimeType as guessReceiptMimeType,
  listReceiptFiles,
  receiptFilesDir,
  receiptFilesKey,
...
export function buildApp(
  ...
  function receiptFilesKeyFor(receipt: { date: string; disambiguator: string; providerId: number }): string {
  ...
  app.get<{ Params: { id: string; filename: string } }>(
    ...
    async (request, reply) => {
      ...
      reply.hijack();
      reply.raw.writeHead(200, { "content-type": guessReceiptMimeType(filename) });
      createReadStream(filePath).pipe(reply.raw);
    },
  );
